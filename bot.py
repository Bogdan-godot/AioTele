from asyncgram.types import MessageObject, GetChat, CommandObject, GetMe
import aiohttp
import asyncio

from asyncgram import loggers
...
class Bot:
    def __init__(self, TOKEN: str):
        ...
        self.handler = CommandHandler(token=self.token)
    ...
    async def send_message(self, chat_id: int, message: str, parse_mode="HTML", reply_markup=None):
    ...
    async def send_photo(self, chat_id: int, file_path: str, caption: str = None, parse_mode="HTML", reply_markup=None):
        await self.start_session()
        ...
        try:
            ...
            with open(file_path, "rb") as photo_file:
                ...
                async with self.session.post(f"{self.url}sendPhoto", data=form_data) as response:
                    ...
                    loggers.bot.info("Сообщение успешно отправлено.")
                    data = await response.json()
                    return data.get("result")
        ...
        async with self.session.get(self.url + "getUpdates", params={"offset": self.update_offset}) as response:
            if response.status == 200:
                ...
                for update in data.get("result", []):
                    ...
                    await self.handler.handle(update, self)
            else:
